#ifndef SJTU_LINKEDHASHMAP_HPP_
#define SJTU_LINKEDHASHMAP_HPP_
...
auto rehash (T value) -> unsigned {
  if ((value & pow2[2]) == 0) return value ^ 1162192769;
  return value ^ 3717357010;
}
...
> class linked_hashmap {
 ...
 public:
  ...
  class iterator {
   public:
    ...
    iterator () = default;
    iterator (ListNode *node, linked_hashmap *home) : node_(node), home_(home) {}
    auto operator++ (int) -> iterator {
      if (node_ == &home_->pivot_) throw invalid_iterator();
      auto node = node_;
      node_ = node_->next_;
      return { node, home_ };
    }
    ...
    auto operator-- (int) -> iterator {
      if (node_ == home_->pivot_.next_) throw invalid_iterator();
      auto node = node_;
      node_ = node_->prev_;
      return { node, home_ };
    }
  ...
  class const_iterator {
   public:
    using difference_type = std::ptrdiff_t;
    using value_type = const linked_hashmap::value_type;
    using pointer = value_type *;
    using reference = value_type &;
    using iterator_category = std::output_iterator_tag;

    const_iterator () = default;
    const_iterator (const ListNode *node, const linked_hashmap *home) : node_(node), home_(home) {}
  ...
  auto operator= (const linked_hashmap &other) -> linked_hashmap & {
  ...
  auto at (const Key &key) const -> const Value & {
    return const_cast<linked_hashmap *>(this)->at(key);
  }
  ...
  auto erase (iterator pos) -> void {
    if (pos == end() || pos.home_ != this) throw 1;
  ...
  auto find (const Key &key) const -> const_iterator {
    return const_cast<linked_hashmap *>(this)->find(key);
  }
 ...
 private:
  ...
  auto growIfNeeded_ () -> void {
    if ((size_ + 1) * kThreshold_ > pow2[capacity_]) grow_();
  }
  auto grow_ () -> void {
    if (capacity_ == 0) {
      capacity_ = 2;
    ...
    ListNode *prospective = new ListNode[pow2[newCapacity]];
    ListNode *node = &pivot_;
    for (int i = 0; i < size_; ++i) {
      node = node->next_;
